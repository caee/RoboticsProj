...
#Magic factors to compensate for weird motor slip, height differences and skews
xfactor=0.3
yfactor=1.1
zfactor=25 #compensates for height difference
for i in range(0,len(d_camera)):
    #OFFSET from image center coordinates to robot coords

    d[i,0]=x_off-d_camera[i,1]
    d[i,1]=-d_camera[i,0]*yfactor #Yfactor added to compensate for skew from motors
    d[i,2]=z_off+h_mm+zfactor-d_camera[i,1]*xfactor #xfactor added to compensate for weird z drift
